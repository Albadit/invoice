...
interface EditClientModalProps {
  ...
  onSave: (clientData: {
    ...
    city: string; zipCode: string; country: string; taxId: string; notes: string;
  }) => Promise<void>;
  client: Client | null;
}
...
export function EditClientModal({ isOpen, onClose, onSave, client }: EditClientModalProps) {
  ...
  useEffect(() => {
    if (client) {
      ...
      setStreet(client.street || ''); setCity(client.city || ''); setZipCode(client.zip_code || '');
      setCountry(client.country || ''); setTaxId(client.tax_id || ''); setNotes(client.notes || '');
    }
  }, [client]);

  async function handleSave() {
    setSaving(true);
    try { await onSave({ name, email, phone, street, city, zipCode, country, taxId, notes }); }
    catch {} finally { setSaving(false); }
  }

  return (
    <FormModal isOpen={isOpen} onClose={onClose} title={t('clients.editClient')} onSave={handleSave} saving={saving} saveLabel={tCommon('actions.save')} saveDisabled={!name.trim()}>
