...
export default function DashboardKpi({ items }: DashboardKpiProps) {
  return (
    <dl className="grid w-full grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4">
      {items.map(
        ({ title, value, subtitle, change, changeType, iconName, onViewAll, viewAllLabel }, index) => (
          <Card
            key={index}
            className="flex flex-col border border-default-200 dark:border-default-100 shadow-sm">
